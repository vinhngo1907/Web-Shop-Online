<?php 
    ...
    $sql_laptop = mysqli_query($conn,"select * from hanghoa where hanghoa.manhom='$_GET[id]' ");
                        ...
                        <div class="collection-filter" id="list-product">
                            <div class="category-products clearfix">
                                <div class="products-grid clearfix">
                                    <?php 
                                        // $sql_laptop=mysqli_query($conn,"select * from hanghoa where hanghoa.manhom='$id_nhom' order by mshh asc");
                                        if($sql_laptop){ 
                                            while($dong_laptop=mysqli_fetch_array($sql_laptop)){
                                    ?>
                                    <div class="col-md-3 col-lg-3 col-xs-6 col-6">
                                        <div class="product-lt">
                                            <div class="lt-product-group-image">
                                                <a href="index.php?xem=chitietsp&id=<?php echo $dong_phone['mshh'] ?>" title="">
                                                    <img class="img-p" src="admincp/modules/quanlyhh/uploads/<?php echo $dong_laptop['hinhanh'] ?>" alt="">
                                                </a>
                                                <?php if($dong_laptop['sale_off'] == 0){ ?>
                                                <?php }else{?>
                                                <div class="giam-percent">
                                                    <span class="text-giam-percent"><?php echo 'Giảm -'.$dong_laptop['sale_off'].'%'; ?></span>
                                                </div>
                                                <?php }?>
                                            </div>
                                            <div class="lt-product-group-info">
                                                <a href="index.php?xem=chitietsp&id=<?php echo $dong_phone['mshh'] ?>" title="">
                                                    <h3><?php echo $dong_laptop['tenhh'] ?></h3>
                                                </a>
                                                <div class="price-box">
                                                    <?php if($dong_laptop['sale_off'] == 0){ ?>
                                                    <p class="special-price">
                                                        <span class="price"><?php echo number_format($dong_laptop['giagoc']).' đ' ?></span>
                                                    </p>
                                                    <?php }else{ ?>
                                                    <p class="old-price">
                                                        <span class="price" style="color: #ccc"><?php echo number_format($dong_laptop['giagoc']).' đ' ?></span>
                                                    </p>
                                                    <p class="special-price">
                                                        <span class="price"><?php echo number_format($dong_laptop['giagoc']-($dong_laptop['giagoc']*$dong_laptop['sale_off']/100)).' đ' ?></span>
                                                    </p>
                                                    <?php }?>
                                                </div>
                                                <div class="clear"></div>
